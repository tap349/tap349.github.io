...
when we cook we **search** for node on chef-zero server:
...
it's possible to make knife-zero use IP instead of FQDN for connecting
to the node by specifying `-a, --attribute` option
...
or set attribute value in _knife.rb_:
