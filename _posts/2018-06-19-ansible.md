...
> <https://docs.ansible.com/ansible/2.6/reference_appendices/playbooks_keywords.html#play>
>
> pre_tasks
...
1. <https://docs.ansible.com/ansible/2.6/user_guide/playbooks_variables.html>
...
=> use _vars/_ directory for almost all role app variables (since they are not
...
### don't use global variables in templates

1. <https://stackoverflow.com/a/38653180/3632318>

pass them as local variables using `vars` option instead:

```yaml
# roles/myapp_app/tasks/secrets.yml

- name: Create secrets file
  template:
    src: "{{ env }}.secret.exs.j2"
...
NOTE: passing `app_name` local variable here might be not required - sometimes
      it's be better to hardcode application name inside the template if this
      template is stored inside application role and is not parameterized from
      outside this role.
