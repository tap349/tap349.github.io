...
variables included with `included_vars` are always hoisted within the file so
...
treat it like a global variable which is available everywhere, also
...
still it's beneficial to pass some ad-hoc variables into the template to cut
down on typing. treat template variables as a way to increase readability of
the template - in this regard passing global variables doesn't look necessary
since they are already short and clear.
...
in the end I decided to prefix all role variables (both passed to the role from
outside and defined inside in _vars/*.yml_ files) with a role name and separate
this prefix with double underscore:
...
=> ALL variables inside particular role MUST have this role prefix!

the only exception is `env` variable which I've decided to make available
globally.

still local variables to the template must be passed without a role prefix
- I hope these variables stay local to this template and cannot sneak into
other roles potentially causing mayhem there.

it's not allowed to use role-prefixed variables inside the template - now
the rule to always pass local variables to the template seems valid again.
