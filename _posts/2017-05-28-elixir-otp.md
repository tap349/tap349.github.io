...
(also passing specified start argument in case of tuple).

you cannot pass more than 1 start argument to `MyApp.Foo.child_spec/1`
(and consequently when using tuple too) - this is because some GenServer
will be supervised eventually (supervisors and agents are GenServers too)
and `GenServer.start_link/3` accepts only 1 argument which is then passed
down to `GenServer.init/1`.

supervisor can also supervise an arbitrary module that is not GenServer
(say, `MyApp.Bar`) by using its child spec:
...
- provided my module itself
  ...
  in theory generated child spec might instruct supervisor to start
  module using another start function (not necessarily `start_link`)
  but I guess this is highly not recommended.

  still, since `child_spec/1` has arity 1, it allows to pass only 1
  start argument to underlying start function.
