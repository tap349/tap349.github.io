...
passed variables propagate to all included roles: e.g., role A includes
role B, role B includes role C. if role A passes `foo` variable to role B,
this variable is available in role C as well without passing it to role C
explicitly - this is true for both `include_role` and `import_role` modules.
...
treat it like a global variable which is available everywhere.
...
it's necessary if these variables are used/included in many places or
they contain lots of data which might be hard to inline (say, SSH keys).

don't extract, say, packages to install or directories to create which
are used in one task only:
...
inside the template if passed variable has the same name as the variable
it resolves to (say, `app_user: "{{ app_user }}"`) or maybe just has the
...
the first solution is not to pass variables to included role explicitly
since they'll be available inside it in any case. this is not the best
solution since it's always better to be explicit about role parameters.

another solution is to use `import_role` instead of `include_role` -
most likely the former directive works because of its static nature
(it must be a dynamic variable resolution that leads to recursive loop):
...
the last solution is not perfect either - it might also lead to recursive loop.

I've found the pattern: infinite recursion occurs when variable with existing
name (say, `app_user: "{{ app_user }}"`) is passed to included role more than
1 level deep - say, role A → (includes) role B → role C and `foo` variable is
passed from A to B and then from B to C:
...
for some reason it helps to replace `include_role` with `import_role`
in role B to avoid recursive loop:
...
use `import_role` instead of `include_role` when passing variable with
existing name - it's okay to pass variable with existing name to template.
