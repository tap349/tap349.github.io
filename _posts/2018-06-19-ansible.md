...
variables propagate to all nested roles: say, role A includes role B, role B
includes role C. if role A passes `foo` variable to role B, this variable is
available in role C as well without passing it to role C explicitly.
