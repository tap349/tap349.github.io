...
### assets (both CSS and JS) are not served when using any javascript driver
...
### `selenium` sometimes fails to locate elements

Capybara sometimes fails to locate elements (links in my case) on page.
increasing `Capybara.default_max_wait_time` doesn't help.

for some reason it doesn't happen when using `webkit`.

### most likely `find_field` doesn't work with submit inputs

### persisted data is not available in tests

sure there are might be different reasons for this including wrapping tests
