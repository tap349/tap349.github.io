...
also when I import another role I define its parameters in _vars/_ directory
as well (in a separate file named after imported role) - instead of passing
these parameters to the role directly in `vars` section.
...
side imported role verifies these variables are defined.
...
{% raw %}
```
$ ansible-playbook -i inventories/prod/hosts site.yml -v
...
