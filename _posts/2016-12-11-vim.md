...
this happens only when file is saved multiple times in quick
succession AND `htop` is opened in iTerm2 (or another terminal) -
somehow this freezes both iTerm2 and MacVim.

**solution**

just try not to open `htop` in the background when working in MacVim.
