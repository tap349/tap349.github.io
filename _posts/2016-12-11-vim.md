...
### location list becomes blank after opening any Markdown file

this happens when trying to open only Markdown files from location
list with search results.

**solution**

the problem is caused by ALE: by default all possible linters are
enabled for all filetypes, also I have `g:ale_lint_on_enter = 1` -
it looks like ALE tries to run some linter for Markdown file when
it's opened from location list and clears location list as a result.

_vimrc_:

```vim
" either enable explicit behaviour:
let g:ale_linters_explicit = 1

" or disable all Markdown linters explicitly:
let g:ale_linters = {
      \   'markdown': []
      \ }
```
