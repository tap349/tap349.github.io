...
### (how to) specify that a field returns a scalar type

NOTE: this is true for both queries and mutations.

you can use empty curly braces in `graphql` gem (1.7.9):
...
but it's no longer a valid syntax in `graphql` gem (1.7.14) -
omit curly braces at all when query should return a scalar:
...
sample GraphQL response for this query:
...
don't use empty curly braces in the query when it returns a scalar
(see the tip above):
