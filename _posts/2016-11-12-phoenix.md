...
- request format must be supported in endpoint configuration
  ...
  NOTE: `accepts` plug in router lists MIME types supported in `Content-Type`
        request header while `render_errors` options lists MIME types supported
        in `Accept` request header (my guess) since `accepts` plug deals with
        request while `render_errors` option deals with response.
