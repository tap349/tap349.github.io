...
### rendering errors with ErrorView

<https://stackoverflow.com/questions/30536979> (Jose Valim):

> In :prod, the default is to render error pages, so you should see a page
> rendered by YourApp.ErrorView with the status code.
>
...
>
> In :test, it works like production but, because you are calling your application
> from the test, your test will also crash if your application crashes.

1. <https://hexdocs.pm/phoenix/Phoenix.Endpoint.html>
2. <https://hexdocs.pm/phoenix/custom_errors.html>

- error templates are rendered for formats which are listed in
  `render_errors` option of endpoint configuration in _config/config.exs_
- errors of accepted formats that don't match an existing `render/2`
  clause in `ErrorView` are caught by `template_not_found/2`

to render **both** HTML and JSON error templates it's necessary to:

- allow to render errors for both formats
...
- add `render/2` clauses for both formats
...
- modify `template_not_found/2` to process both formats
