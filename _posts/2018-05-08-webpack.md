...
when importing Sass files, `~` prefix in front of imported file is used
to tell that this is not relative import - it's not relative to current
file but relative to _node\_modules/_ directory.
...
don't add `resolve-url-loader` package (see notes above).
