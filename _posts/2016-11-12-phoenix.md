...
so any template will be precompiled into corresponding `render/2` function
inside view - there is no need to create such functions manually. though it's
possible in order to render different template or return text instead as it's
done in `ErrorView` module:
