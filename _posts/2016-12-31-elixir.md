...
> Elixir is pattern matching params first (the passed in map is the right-hand
> side, params becomes the left), then pattern matches user_id, as the
> left-hand side, against params which is now the right-hand side, like so.
...
the match will be successful only if `user_id` key in `params` map
has the value `2`.
