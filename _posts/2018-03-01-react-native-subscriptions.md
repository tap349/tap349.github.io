...
- sign out of Apple ID on iPhone before testing subscriptions
  ...
  occasionally `Apple ID Verification` alert dialog might appear that asks
  to enter password for test account in `Settings` - it's safe to ignore it
  and press `Not Now` button all the time.
