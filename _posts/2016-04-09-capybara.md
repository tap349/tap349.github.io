...
**NOTE**: only methods (matchers, finders, etc.) that work with specific elements
          use attribute values and texts to find corresponding elements -
          generic methods (like `find`) use CSS or XPath selectors only.
...
solution: use database truncation instead of transactions
  ...
  config.before(:suite) do
    if config.use_transactional_fixtures?
      raise(<<-MSG)
        ...
        let DatabaseCleaner decide whether it's necessary to do it or not.
      MSG
    end
