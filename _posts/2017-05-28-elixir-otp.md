...
- child spec map (`MyApp.Foo.child_spec(arg)`)
...
- provided my module itself
  ...
  in this case `MyApp.Bar` module must implement `child_spec/1` that
...
- *[DEPRECATED]* generated by `Supervisor.Spec` helpers
  ...
  in this case `MyApp.Bar` module must implement `start_link`
