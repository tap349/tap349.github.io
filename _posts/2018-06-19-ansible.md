...
variables are always referenced using {% raw %}`{{ var }}`{% endraw %} syntax
of Jinja2 templating system - even if they are not interpolated inside another
string:
...
### (how to) override `changed` result
...
>
> YAML syntax requires that if you start a value with {{ foo }} you quote the
> whole line, since it wants to be sure you aren’t trying to start a YAML dictionary.
