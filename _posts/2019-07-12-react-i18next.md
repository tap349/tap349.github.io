...
it's possible to specify locale (aka full locale) as a language to use (`lng`
configuration option) and use resources with language-only keys (`resources`
configuration option) at the same time - correct language will be picked up
automatically:
