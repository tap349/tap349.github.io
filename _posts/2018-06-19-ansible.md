...
variables are always referenced using {% raw %}`{{ }}`{% endraw %} and must
be always surrounded with double quotes when not interpolated inside another
string (see the tip below):
...
### when to surround strings with double quotes
...
=> surround a string with double quotes only when it contains variables:

{% raw %}
```yaml
elixirapp_systemd:
  app_service: "{{ elixirapp_app_name }}_{{ env }}"
```
{% endraw %}

don't surround paths (strings containing backslashes or forward slashes)
with double quotes:
...
- with default values
  ...
  omit this parameter when executing the role and use provided default value
  from _defaults/main.yml_ - see the tip above.
...
specify optional parameters only once - don't add optional parameters with
default values to `*_optional_vars` and vice versa:
