...
### don't use common app variables
...
it might be not very dry but so far it doesn't seem to cause any trouble.
...
NOTE: passing `app_name` local variable here might be not required - sometimes
      it's be better to hardcode app name inside the template if this template
      is stored inside app role and is not parameterized from outside this role.
